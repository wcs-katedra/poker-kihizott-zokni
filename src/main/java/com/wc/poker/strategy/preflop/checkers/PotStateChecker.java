...
//	private static List<Hand> raiseAtEarlyGamePhaseEarlyPositionLimped = new ArrayList<>();
	...
	private void multiplier(Multiplier m) {
		range = (int) (RangeLoader.getOtherHandRange().size() * m.getMultiplier());
	}
	...
	public void checkPotStatusEGPEP() {
		switch (potstatus) {
			case CLEAN:
			case LIMPED: {
				multiplier(Multiplier.TWENTY);
				PreFlopStrategy.setHandsToUse(RangeLoader.getOtherHandRange().subList(0, range));
